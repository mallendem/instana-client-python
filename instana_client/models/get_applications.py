...
class GetApplications(BaseModel):
    ...
    endpoint_types: Optional[List[StrictStr]] = Field(default=None, alias="endpointTypes")
    ...
    name_filter: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=256)]] = Field(default=None, description="filter by name with `contains` semantic. Eg: Let's say there are 2 names `app1` and `app2`, you can set `app`` here to include the two names", alias="nameFilter")
    ...
    __properties: ClassVar[List[str]] = ["applicationBoundaryScope", "applicationId", "endpointId", "endpointTypes", "metrics", "nameFilter", "order", "pagination", "serviceId", "technologies", "timeFrame"]
    ...
    def endpoint_types_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['UNDEFINED', 'RPC', 'EVENT', 'GRAPHQL', 'BATCH', 'SHELL', 'HTTP', 'SDK', 'OPENTELEMETRY', 'INTERNAL', 'RPG', 'DATABASE', 'MESSAGING', 'PAGE', 'PAGE_RESOURCE']):
                raise ValueError("each list item must be one of ('UNDEFINED', 'RPC', 'EVENT', 'GRAPHQL', 'BATCH', 'SHELL', 'HTTP', 'SDK', 'OPENTELEMETRY', 'INTERNAL', 'RPG', 'DATABASE', 'MESSAGING', 'PAGE', 'PAGE_RESOURCE')")
        return value
