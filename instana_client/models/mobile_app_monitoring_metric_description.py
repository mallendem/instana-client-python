...
class MobileAppMonitoringMetricDescription(BaseModel):
    ...
    beacon_types: Annotated[List[Annotated[str, Field(min_length=1, strict=True)]], Field(min_length=1)] = Field(alias="beaconTypes")
    ...
    formatter: Annotated[str, Field(min_length=1, strict=True)] = Field(description="* `NUMBER`: Generic number * `BYTES`: Number of bytes * `KILO_BYTES`: Number of kilobytes * `MEGA_BYTES`: Number of megabytes * `PERCENTAGE`: Percentage in scale [0,1] * `PERCENTAGE_100`: Percentage in scale [0,100] * `PERCENTAGE_NO_CAPPING`: Percentage in scale [0,1] but value could exceed 1 for example when metric is aggregated * `PERCENTAGE_100_NO_CAPPING`: Percentage in scale [0,100] but value could exceed 100 for example when metric is aggregated * `LATENCY`: Time in milliseconds, with value of 0 should not be considered a a strict 0, but considered as < 1ms * `NANOS`: Time in nanoseconds * `MILLIS`: Time in milliseconds * `MICROS`: Time in microseconds * `SECONDS`: Time in seconds * `RATE`: Number of occurrences per second * `BYTE_RATE`: Number of bytes per second * `UNDEFINED`: Metric value unit is not known ")
    label: Annotated[str, Field(min_length=1, strict=True)] = Field(description="The name of the metric. For example, `Call count`, `Erroneous calls`, `Service count` etc.")
    metric_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="The unique id of the metric. For example, `calls`, `erroneousCalls`, `latency` etc.", alias="metricId")
    ...
    secondary_beacon_types: Optional[List[StrictStr]] = Field(default=None, alias="secondaryBeaconTypes")
    tag_name: Optional[StrictStr] = Field(default=None, alias="tagName")
    __properties: ClassVar[List[str]] = ["aggregations", "beaconTypes", "defaultAggregation", "description", "formatter", "label", "metricId", "pathToValueInBeacon", "secondaryBeaconTypes", "tagName"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if path_to_value_in_beacon (nullable) is None
        # and model_fields_set contains the field
        if self.path_to_value_in_beacon is None and "path_to_value_in_beacon" in self.model_fields_set:
            _dict['pathToValueInBeacon'] = None

        # set to None if secondary_beacon_types (nullable) is None
        # and model_fields_set contains the field
        if self.secondary_beacon_types is None and "secondary_beacon_types" in self.model_fields_set:
            _dict['secondaryBeaconTypes'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "secondaryBeaconTypes": obj.get("secondaryBeaconTypes"),
