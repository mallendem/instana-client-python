...
class ServiceConfig(BaseModel):
    ...
    id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="A unique string for the service configuration. Eg: `G510hmXYSDysLZ5kuj0BaQ`")
    label: Annotated[str, Field(min_length=1, strict=True)] = Field(description="It contains the tags defined in `matchSpecification` concatenated with a dash. Eg: if the `matchSpecification` contains keys `kubernetes.namespace.name` and `docker.label`, `label` would be `kubernetes.namespace.name-docker.label`. ")
